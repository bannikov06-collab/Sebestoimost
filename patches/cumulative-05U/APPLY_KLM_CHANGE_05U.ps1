...
$PatchFiles = @(
  ...
  'app\lib\cmlStandardRules.ts',
  'app\lib\sidewallRules.ts',
  'app\lib\materialPlanning.ts',
  'app\lib\procurementExportRules.ts',
  'tests\element-resource-export.test.mjs',
  'docs\CHANGE_05T_ELEMENT_RESOURCE_EXPORT.md',
  'docs\CHANGE_05U_5P_CML_STANDARD_RULES.md'
)
$Changed = @($PatchFiles + 'app\components\PreliminarySpecificationPanel.tsx')
...
try {
  ...
  $Checkpoint = Join-Path $CheckpointRoot ('before-change-05U-' + $Stamp)
  ...
  Show-Step 'KLM v32 - CUMULATIVE CHANGE 05U 5P AND CML STANDARD RULES'
  if (-not $Project) {
  ...
  $Stage = 'preflight'
  Show-Step '0/6 Preflight - verify cumulative CHANGE 05T R1 source'
  $RequiredBase = @(
    'app\page.tsx',
    'app\components\MaterialsPlanningPanel.tsx',
  ...
  $PlanningText = Get-Content -LiteralPath (Join-Path $Project 'app\components\MaterialsPlanningPanel.tsx') -Raw -Encoding UTF8
  if ($PageText -notmatch 'selectedResourceElementIds' -or $PageText -notmatch 'exportElementResourceSpecification' -or $PlanningText -notmatch 'downloadProcurementComparisonXlsx' -or $ManufacturingText -notmatch 'isJointSpecificationRow') { throw 'CHANGE 05T R1 cumulative working source was not detected. Update stopped before changing files.' }
  Write-Host 'Preflight OK: cumulative CHANGE 05T R1 source detected. CHANGE 05U adds the corrected 5P package rule, confirms 6x200 for CD 2000 A, and enables standard CML 4P/5P from 630 to 6300 A. Existing D1/R2 bindings will be reused; no migration is required.' -ForegroundColor Green
  ...
  $Stage = 'targeted 5P preliminary update'
  $TargetedScript = Join-Path $Patch 'scripts\enable-confirmed-5p-preliminary.mjs'
  if (-not (Test-Path -LiteralPath $TargetedScript)) { throw ('Targeted update script is missing: ' + $TargetedScript) }
  & node.exe $TargetedScript $Project
  if ($LASTEXITCODE -ne 0) { throw ('Targeted 5P preliminary update failed. Exit code: ' + $LASTEXITCODE) }
  ...
  try {
    ...
    if ($stagedExit -eq 0) {
      ...
      & $GitExe commit -m 'KLM v32 CHANGE 05U: 5P and standard CML rules'
  ...
  Write-Host ('Checkpoint: ' + $Checkpoint)
  Write-Host 'Installed: cumulative CHANGE 05U. For 5P every package contains four working conductors plus its own PE bus; PET remains limited to A/B/C/N. CD 2000 A uses 6x200. CML 4P/5P is enabled for the standard 630-6300 A range. Joint bolts remain selected by KD: M12x130 for 4P and M12x140 for 5P. CHANGE 05P-05T R1 are included. Existing D1/R2 bindings are preserved.'
