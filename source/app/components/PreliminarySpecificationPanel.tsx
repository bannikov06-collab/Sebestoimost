...
export default function PreliminarySpecificationPanel({ specifications, setSpecifications, estimate, onImportFiles, onSave, onDelete, onConvert }: Props) {
  ...
  return <div className="preliminary-specification">
    <div className="notice ok-notice"><b>Коммерческий расчёт</b><span>S1/S2/S3 по умолчанию равны 750/1500/2500 мм. Для каждой строки длину можно изменить. Наценка по умолчанию 22%, допускается 0%.</span></div>
    ...
    {active && result && <>
      ...
      <section className="panel"><div className="panel-title"><div><span>04</span><h2>Расчёт по позициям</h2></div><small>{rows.length} показано · {active.rows.length} всего</small></div><div className="table-toolbar"><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Поиск по артикулу или наименованию" aria-label="Поиск по предварительной спецификации" /></div><div className="table-wrap"><table className="preliminary-table"><thead><tr><th>№</th><th>Артикул / наименование</th><th>Кол-во</th><th>Класс длины</th><th>Расчётная длина</th><th>Себестоимость за шт.</th><th>Сумма</th><th>Статус</th></tr></thead><tbody>{rows.map((row) => { const calculated = result.rows.find((item) => item.rowId === row.id); return <tr key={row.id} className={calculated?.status === "excluded" ? "mapping-row" : ""}><td>{row.rowNumber}</td><td><b>{row.article}</b><small>{row.item}</small></td><td>{row.quantity} {row.unit}</td><td>{row.lengthClass || (row.commercialLengthMm === 3000 ? "Стандарт" : "—")}</td><td>{row.lengthClass ? <input type="number" min="1" step="1" value={row.commercialLengthMm} onChange={(event) => updateLength(row.id, event.target.value)} aria-label={`Длина ${row.article}`} /> : row.commercialLengthMm ? `${row.commercialLengthMm} мм` : "по типовому правилу"}</td><td>{calculated?.status === "calculated" ? money.format(calculated.unitCost) : "—"}</td><td>{calculated?.status === "calculated" ? money.format(calculated.totalCost) : "—"}</td><td><span className={`planning-status ${calculated?.status === "calculated" ? "covered" : "mapping"}`}>{calculated?.status === "calculated" ? "Рассчитано" : "Не включено"}</span><small>{calculated?.reason}</small></td></tr>; })}</tbody></table></div></section>
    </>}
    {!specifications.length && <section className="empty-workspace"><span>Коммерческий расчёт появится здесь</span><h2>Загрузите предварительную спецификацию</h2><p>Система распознает артикулы KLM, назначит расчётные длины S1/S2/S3 и покажет себестоимость, наценку, НДС и договорную стоимость.</p></section>}
  </div>;
}
